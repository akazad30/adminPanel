        ...
        <div class="graphs">
		   ...
		   <div class="widget_2">
						...
						<div class="panel-body">
							<form role="form" class="form-horizontal" method="post" action="Edited_About.php" enctype="multipart/form-data">
									...
									<div class="col-md-8">
										<div class="input-group input-icon right">
											...
											<input id="email" class="form-control1" type="text" name="ParentId" value="<?php
																...
																$stmt = $DB_con->prepare("SELECT HeadingDesc,ParentId from heading where ParentId = '".$_GET['ParentId']."' LIMIT 1 ");
																$stmt->execute();
																if($stmt->rowCount() > 0)
																{
																	while($row=$stmt->fetch(PDO::FETCH_ASSOC))
									...
									<div class="col-md-8">
										<div class="input-group input-icon right">
											...
											<input id="email" class="form-control1" type="text" name="HeadingTitle" value="<?php
																...
																$stmt = $DB_con->prepare("SELECT HeadingTitle from heading where ParentId = '".$_GET['ParentId']."' LIMIT 1 ");
																$stmt->execute();
																if($stmt->rowCount() > 0)
																{
																	while($row=$stmt->fetch(PDO::FETCH_ASSOC))
									...
									<div class="col-md-8">
										<div class="input-group input-icon right">
											...
											<input id="email" class="form-control1" type="text" name="HeadingStatus" value="<?php
																...
																$stmt = $DB_con->prepare("SELECT status from heading where ParentId = '".$_GET['ParentId']."' LIMIT 1 ");
																$stmt->execute();
																if($stmt->rowCount() > 0)
																{
																	while($row=$stmt->fetch(PDO::FETCH_ASSOC))
									...
									<div class="col-md-8">
										<div class="input-group">							
											<input type="file" name="Image" value="imagechek" /><?php
																...
																$stmt = $DB_con->prepare("SELECT UserPic from image where ParentId = '".$_GET['ParentId']."' LIMIT 1 ");
																$stmt->execute();
																if($stmt->rowCount() > 0)
																{
																	while($row=$stmt->fetch(PDO::FETCH_ASSOC))
