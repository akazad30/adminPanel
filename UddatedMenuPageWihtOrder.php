<?php
	...
	if(isset($_POST['submit']))
	{
		...
		if(!isset($errMSG))
		{
			...
			$p = "UPDATE heading SET HeadingDesc=:headingdesc, Status=:status, HeadingTitle=:headingtitle WHERE HeadingId = '".$_POST['ParentId']."' ";
				...
				$q = "UPDATE description SET ContentDesc=:headingdesc, Status=:status, HeadingTitle=:headingtitle WHERE HeadingId= '".$_POST['ParentId']."' ";
					$query = $DB_con->prepare($q);
					$query->execute(array(
						':headingdesc'     => $HeadingDescription,
						':status'   => $HeadingStatus,
						':headingtitle'    => $HeadingTitle
						//':pID'   => $ParentId
					));
				...
				$r = "UPDATE image SET UserPic=:upic, Status=:status WHERE HeadingId= '".$_POST['ParentId']."' ";
